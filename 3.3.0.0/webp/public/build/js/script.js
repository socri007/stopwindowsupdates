...
const cssColors = (color) => {
	//alert(getComputedStyle(document.documentElement).getPropertyValue(color));
  return getComputedStyle(document.documentElement).getPropertyValue(color)
}

const getColor = () => {
  return window.localStorage.getItem('color') ?? 'normal'
}

const colors = {
  primary: cssColors(`--color-${getColor()}-100`),
  primaryLight: cssColors(`--color-${getColor()}-400`),
  primaryLighter: cssColors(`--color-${getColor()}-500`),
  primaryDark: cssColors(`--color-${getColor()}-600`),
  primaryDarker: cssColors(`--color-${getColor()}-700`),
}
